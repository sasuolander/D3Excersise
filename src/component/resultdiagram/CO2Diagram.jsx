...
export default class CO2Diagram extends PureComponent {
  ...
  static getDerivedStateFromProps(nextProps, State) {
    ...
    xScale.domain(
      extent(nextProps.data, d => {
        return d.year;
      })
    );
    yScale.domain([
      0,
      max(nextProps.data, d => {
        return d.value;
      })
    ]);
  ...
  render() {
    ...
    select(this.GMargin.current).attr(
      "transform",
    ...
    return (
      <svg width={width} height={height}>
        <g ref={this.GMargin}>
          <Axis x={0} y={0} scale={yScale} call={yAxis} type="Left" />
          <Axis
            x={0}
            y={heightUsed}
            scale={xScale}
            ...
            type="Bottom"
          />
          <Path xScale={xScale} yScale={yScale} data={data} />
          <Label
            position={"x"}
            width={widthUsed}
            height={heightUsed}
